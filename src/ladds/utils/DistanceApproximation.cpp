/**
 * @file DistanceApproximation.cpp
 * @author P. Gomez
 * @date 18.08.22
 */
...
std::tuple<std::array<double, 3>, std::array<double, 3>> distanceByLinearInterpolation(const Particle &i,
  ...
  // Compute actual distance by propagating along the line to time t
  const auto p1PosAtTimeT = add(old_r_i, mulScalar(vi, t));
  const auto p2PosAtTimeT = add(old_r_j, mulScalar(vj, t));
